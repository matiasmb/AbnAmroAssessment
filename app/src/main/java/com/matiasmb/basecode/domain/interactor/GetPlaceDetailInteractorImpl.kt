...
class GetPlaceDetailInteractorImpl @Inject constructor(
    ...
    override suspend fun fetchPlaceDetails(placeId: String): Flow<PlaceView?> {
        return flow {
            placesApiService.getPlacesDetails(placeId).collect {
                when(it) {
                    ResponseType.Failure -> emit(null)
                    is ResponseType.Success -> emit(transformResult(it.data))
                }
            }
        }
    }
